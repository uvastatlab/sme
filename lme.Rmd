...
We specified the formula within the `lmer()` function as `outcome ~ fixed effect + (random slope | random intercept)`. Since we have set the random slope to 1, we have a fixed slope. In other words, all cases will have the same slope estimated. We specified the random intercept as `block` which means that the intercept will be allowed to vary by `block`. We can now inspect the model estimates and its overall fit. [@faraway_book] Does `treat` appear to affect the number of eggs, and if so, which treatment seems superior?
...
We see that the means of treatment `E` and `F` are closer to each other than either of them are to treatment `O`.

We can confirm this using pairwise comparisons, adjusted p-values and confidence intervals:
...
This plot shows us the spread of predicted egg values based on their treatment group. We can see the same trend as indicated by their means, `E` and `F` are more similar to one another than either is to `O`, and we can also see that the spread of predicted values for `O` overlaps with `E` and `F`'s spreads. We can now assess model fit and significance of predictors.
...
```{r}
caterpillarPlot(m, grouping = "block")
m_assum <- mlm_assumptions(m)
m_assum$linearity.plots
m_assum$homo.test
...
qplot(time,level, data=phosphateL, geom=c("line","point"), facets=~id)
```
...
```{r}
# Null model with random effects for intercept and slope
m <- lmer(level ~ 1 + (time | id), data = phosphateL)
# Adding in a predictor
m2 <- lmer(level ~ group + (time | id), data = phosphateL)

m3 <- lmer(level ~ group + time + (time | id), data = phosphateL)
m4<-lmer(level ~ group*time + (time| id), data=phosphateL)

anova(m, m2, m3, m4)

summary(m4)

qqmath(ranef(m4, condVar=TRUE))  
randoms <- merTools::REsim(m4, n.sims = 500)
