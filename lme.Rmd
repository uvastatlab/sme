...
Before we begin modeling, we make the baseline phosphate measure a predictor since a baseline is technically not a response to any treatment or condition. [@senn] To do this we use `pivot_longer()` again but this time without selecting column t0. When finished we rename the column as "baseline". We now have a column called "baseline" in our long data frame that we will use as a predictor in our models.
...
We can also examine the distribution of residuals by time within each group using the syntax below. The plot looks pretty good. The residuals look evenly distributed and not too far from 0.
...
Looking at the summary for model m2 reveals coefficients that are impossible to interpret. This is one of the drawbacks of models with non-linear effects. However we see under the Random effects section that there is more variation within subjects (0.435) than between subjects (0.258).  
...
Another approach to analyzing longitudinal data is via Generalized Least Squares [@nlmebook]. In this framework we do away with random effects and model _correlated_ errors. The nlme package that comes with R [@nlme] provides the `gls()` function for this task along with a family of Correlation Structure Classes that allow us to model various correlation structures.

To motivate the Generalized Least Squares approach, let's ignore the grouping structure and fit a simple linear model using the `gls()` function.
...
A tool for investigating correlation of errors is the _semivariogram_. Below we use the `Variogram()` function from the nlme package and pipe the result into the `plot()` function. The arguments `form = ~ time | id` and `resType = "n"` says to look at the "normalized" residuals over time by id. In addition we specify `robust = TRUE` to reduce the influence of outliers. The increase in the semivariogram values over "distance" (ie, the positive slope) indicate the errors are correlated.
...
To help understand this, let's extract the residuals from our simple linear model, and then apply the `acf()` function to the residuals by subject id. The `acf()` calculates autocorrelation at all possible lags. We then extract the autocorrelation at lag 1 and sort by absolute value. Notice that more than half the subjects have an autocorrelation at lag 1 greater than 0.2 in absolute value.

```{r}
r <- residuals(m0)
acf_out <- tapply(r, phosphateL$id, acf, plot = FALSE)
lag1 <- sapply(acf_out, function(x)x[1,1][["acf"]])
sort(abs(lag1), decreasing = TRUE)
```

Now we model this correlation using the `corCAR1()` correlation structure. This will estimate a single correlation parameter for the errors that will decrease exponentially with each lag. Notice the model has not changed. We've simply added the argument `correlation = corCAR1(form = ~ time | id)`. 
...
Now the semivariogram shows no pattern, suggesting the model with the correlation structure is adequate.
...
In addition we can formally compare the models using the `anova()` function. The large test statistic and difference in AIC/BIC values provides strong evidence against the assumption of independent errors.
...
Compare to mixed-effect model coefficients.
