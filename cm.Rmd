...
Now we fit a series of zero-inflated models using the `zeroinfl()` function from the pscl package [@pscl]. The model syntax is the same as the syntax used for `lm()` and `glm()`, except now we can specify two models separated by a `|`. 
