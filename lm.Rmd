...
plot(m3, which=1)
```
The fitted vs. residuals plot summarizes the spread of residuals. For a 'good' model, we expect to see residuals centered around 0, with an even distribution above and below 0. The red line is a line fitted to the residuals, so we expect it to be horizontal at 0.

Many of the values on the plot are within 20 units of 0. This is what we would expect based on the residual standard error. The distribution of residuals of a linear regression model should have a mean of 0. This distribution has a standard deviation called the residual standard error. The residual standard error is how much we can expect a prediction to deviate from its true value.

From summarizing the data, we saw that about 50% of the data is in about a 20 unit range (1st quartile: 116 , 3rd quartile: 137.5). So if we can expect the model to predict bp with a 17 unit range, then the model's error is similar to the range of the data. Thus, the model cannot accurately predict bp.
...
These values do not cluster around the slope=1 line, another signal that this model does not perform well. It is likely that factors outisde of the predictors in the model also affect bp. 
