...
Model fracture as a function of the other variables. (Hosmer and Lemeshow 2013, Chapter 2)
...
We can pipe the table into `proportions()` to see a breakdown of proportions.
...
Finally we might want to look at response by year and sex. Below we only look at the Freshman group using the `subset` argument. Setting `drop.unused.levels = TRUE` ensures we only see results for Freshman. We see Males prefer "Defeat" while Females prefer "De-escalate".
...
By definition, the $P(\text{response} <= \text{Withdraw}) = 1$ and thus is not modeled.

We can interpret the coefficients as odds ratios by exponentiating them. But since we have interactions, the main effect coefficients only apply to certain groups. For example: 

- the Male coefficient of -0.9778 applies to Freshman Males. 
- The Sophomore coefficient of 0.3859 applies Freshman Females. 
...
Exponentiating the Male coefficient returns an odds ratio of 0.376. This says the odds of Freshman Males preferring, say, De-escalate or lower is (1 - 0.376 = 0.624) about 62% lower than the odds of Freshman Females preferring De-escalate or lower.

```{r}
exp(coef(m)["sexMale"])
```

Likewise, exponentiating the sum of coefficients for Sophomore Males, -0.8578, returns an odds ratio of 0.424. This says the odds of Sophomore Males preferring, say, De-escalate or lower is (1 - 0.424 = 0.576) about 58% lower than the odds of Sophomore Females preferring De-escalate or lower.
...
We can also use the model to get estimated cumulative probabilities. For example, what is the estimated cumulative probability that Grad Student Females prefer De-escalate or lower? Using our model coefficients, we can write this out as follows:
...
Now we carry out the proportional odds test. A low p-value is evidence against the hypothesis of proportional odds. The Overall test is rejected but the tests for each predictor appear fine. 
...
Checking proportional odds as described in MASS (p. 204)
...
```{r}
library(ggeffects)
ggeffect(m, terms = c("year", "sex")) |> plot()
```

```{r}
library(emmeans)
emmeans(m, ~ year|response, mode = "prob") |>
  plot()
```
