...
We can also use the model to get estimated cumulative probabilities instead of odds rations. For example, what is the estimated cumulative probability that Grad Student Females prefer De-escalate or lower? Using our model coefficients, we can write this out as follows:
...
Another approach to inspecting the proportional odds assumption is to look at the _differences of logits_ of the cumulative probabilities using the relative frequencies, as described in Venables and Ripley (2002, page 204). To begin we calculate cumulative probabilities for each of the 10 possible variable combinations.
