...
```{r}
sf <- nrow(malaria)*0.75
train  <- malaria[1:sf, ]
test   <- malaria[(sf+1):nrow(malaria), ]
```
...
```{r}
lrm <- glm(mal ~ age + log(ab), family = binomial(link = "logit"), data = train)
```
...
Now let's test our model on the training data and see how well it performs. 

```{r}
prediction <- predict(lrm, newdata = test, type="response")
```
